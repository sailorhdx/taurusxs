...
var Layout = function () {
    ...
    var handleSidebarAndContentHeight = function () {
        ...
        if (body.hasClass("page-footer-fixed") === true && body.hasClass("page-sidebar-fixed") === false) {
            ...
            //if (content.height() < available_height) { // comment out by TaurusX, 20170315
                content.css('min-height', available_height);
                
                if ($('.page-content .page-content-body #mainFrame').size() != 0) {// add by TaurusX, 20170312, 根据右侧content高度设置iframe大小
                	$('.page-content .page-content-body #mainFrame').attr('height', available_height - 41 - 18); 
                }
            //} // comment out by TaurusX, 20170315
        } else {
            ...
            if ($('.page-content .page-content-body #mainFrame').size() != 0) {// add by TaurusX, 20170312, 根据右侧content高度设置iframe大小
            	$('.page-content .page-content-body #mainFrame').attr('height', height - 41 - 18); 
            }
        }
    };
    ...
    var handleSidebarMenu = function () {
        ...
        // add by TaurusX, 20170310,   ', li > h3 > a.ajaxify'
        $('.page-sidebar').on('click', ' li > a.ajaxify, li > h3 > a.ajaxify', function (e) {
            e.preventDefault();
            App.scrollTop();
            ...
            $(this).parents('li').each(function () {
                ...
                // add by TaurusX, 20170310, BEGIN
                if ($(this).parent('ul.page-sidebar-menu').size() === 1) {
                    $(this).find('> a').append('<span class="selected"></span>');
                }
                // add by TaurusX, 20170314, END
            });
    ...
    return {
        ...
        loadAjaxContent: function(url, sidebarMenuLink) {
            ...
            if (tabmode === 1 || tabmode === "1") {// 分页
	            $.fn.jerichoTab.addTab({
	                tabFirer: sidebarMenuLink,
	                title: tool_index.trim(sidebarMenuLink.text(),"g"),
	                closeable: true,
	                data: {
	                	dataType:"ajax",
	                	dataLink: sidebarMenuLink.attr("page-href")
	                }
	            }).loadData();
			 	$('#jerichotab_contentholder').attr("style","");
			 	App.stopPageLoading();
	            ...
	            $.ajax({
	                ...
	                error: function (res, ajaxOptions, thrownError) {
	                    App.stopPageLoading();
	                    // pageContent.html(res);
	                    // add by TaurusX,20170224,begin
	                    var status = res.status;
	                    var readyState = res.readyState;
	                    var pageCode = "page-404";
	                    var detailMessage = "<h3>Oops! You're lost.</h3>" 
	                    	+ "<p> We can not find the page you're looking for. <br/> </p>";
	                    if (status !== 404 && status !== "404") {
	                    	pageCode = "page-500";
	                    	if (status === "403" || status === 403){
	                    		detailMessage = "<h3>Oops! Forbidden.</h3>"
		                    		+"<p> Subject does not have permission. <br/> </p>";
	                    	} else if (status === "400" || status === 400){
	                    		detailMessage = "<h3>Oops! Parameter bound error.</h3>"
		                    		+"<p> The server is unable to parse the parameters. <br/> </p>";
	                    	} else {
		                    	detailMessage = "<h3>Oops! Something went wrong.</h3>"
		                    		+"<p> We are fixing it! Please come back in a while. <br/> </p>";
	                    	}
	                    }
	                    	
	                    var pageData = "<div class=\"row\">";
	                    	pageData = pageData + "<div class=\"col-md-12 " + pageCode + "\">";
	                    	pageData = pageData + "<div class=\"number\"> " +status + " </div>"
	                    	pageData = pageData + "<div class=\"details\">";
	                    	pageData = pageData + detailMessage;
	                    	pageData = pageData + "<h4>Could not load the requested content("+readyState+").</h4>";
